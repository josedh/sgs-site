...
const addContact = (contact) => {
    return new Promise((resolve, reject) => {
        ...
        safe.phone = contact.bodyPattern('phone', /^[\+]?[(]?[0-9]{3}[)]?[-\s\.]?[0-9]{3}[-\s\.]?[0-9]{4,6}$/im)
        ...
        let verificationUrl = 'https://www.google.com/recaptcha/api/siteverify?secret=' + captcha_secret + '&response=' + front_end_token + '&remoteip=' + contact.connection.remoteAddress
        let response = {}
...
function saveToPostgres(safe, createdts, updatedts) {
    return new Promise((resolve, reject) => {
        let q = util.format('INSERT INTO %s(name, email, phone, message, captcha_score, acknowledged, created_on, updated_on) VALUES($1, $2, $3, $4, $5, $6, $7, $8)', process.env.PG_CONTACTS_TABLE)
